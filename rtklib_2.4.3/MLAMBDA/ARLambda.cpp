...
VectorXd ARLambda::resolveIA(VectorXd& ambFloat, MatrixXd& ambCov, MatrixXd& F, VectorXd& S, int* ret)
{
	// Check input
	if (ambFloat.size() != ambCov.rows() || ambFloat.size() != ambCov.cols())
	{
	...
	{
		//MatrixXd F; VectorXd S;
		*ret=lambda(ambFloat, ambCov, F, S, 2);
		VectorXd ambFixed = VectorXd::Zero(ambFloat.size());
		if (*ret == 0)
		{
...
//*a     I  float parameters (n x 1)
//*Q     I  covariance matrix of float parameters(n x n)
//*F     O  fixed solutions (n x m)
...
int ARLambda::lambda(VectorXd & a, MatrixXd & Q, MatrixXd & F, VectorXd & s, const int & m)
{
	...
	VectorXd z = VectorXd::Zero(n); 
	MatrixXd Z = MatrixXd::Identity(n, n);//n*nµÄµ¥Î»¾ØÕó
